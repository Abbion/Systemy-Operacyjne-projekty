...
void semafor_ustaw_nr(int id, int val, int sem_nr)
{
    int ustaw_sem;
    ustaw_sem=semctl(id, sem_nr, SETVAL, val);
    if (ustaw_sem==-1)
    {
        perror("Nie mozna ustawic semafora: ");
        exit(EXIT_FAILURE);
...
void semafor_p_2(int id, int sem_1, int sem_2)
{
    int zmien_sem;
    struct sembuf bufor_sem[2];
    bufor_sem[0].sem_num = sem_1;
    bufor_sem[0].sem_op = -1;
    bufor_sem[0].sem_flg = 0;
    bufor_sem[1].sem_num = sem_2;
    bufor_sem[1].sem_op = -1;
    bufor_sem[1].sem_flg = 0;
    zmien_sem=semop(id,bufor_sem,2);
    if (zmien_sem==-1)
    {
        if(errno == EINTR)
        {
...
void semafor_v_2(int id, int sem_1, int sem_2)
{
    int zmien_sem;
    struct sembuf bufor_sem[2];
    bufor_sem[0].sem_num = sem_1;
